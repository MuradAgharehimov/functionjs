...
//     let xyz = results.filter(item => item.order.customerId == "ERNSH");
...
//   let emplo = results.filter(item => item.order.employeeId);
  
//   let emplos = emplo.sort((a, b) => a.order.employeeId - b.order.employeeId);
...
//   let orderD = results.filter(item => item.orderDetails.length > 3);
...
//   let idtap = results.filter(item => item.order.id == 11075)
